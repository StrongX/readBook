...
class CacheHelper {
    ...
    for (int i = 0; i < 10; i++) {
      if (chapters.length > i) {
        ...
        if (await db.chapterIsCache(bookName, chapter['chapterName'])) {
          ...
          XHttp.getWithCompleteUrl(chapter['link'], {},
              (String response) async {
            response = response.replaceAll(RegExp("\r|\n"), "");
            DataHelper db = await getDataHelp();
            await db.cacheChapter(chapter['bookName'], chapter['chapterName'],
                response, chapter['link']);
          });
        }
      }
    }
  ...
  static getChapterContent(String bookName, String chapterName, String link,
      void callBack(String content)) async {
    DataHelper db = await getDataHelp();
  ...
  static updateChapterContent(String bookName, String chapterName, String link,
      void callBack(String content)) async {
    print('request chapter content');
    await XHttp.getWithCompleteUrl(link, {}, (String response) async {
      response = response.replaceAll(RegExp("\r|\n"), "");
